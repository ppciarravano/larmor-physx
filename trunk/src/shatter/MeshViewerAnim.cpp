...
double explode = 1.0;
double explodeAcceleration = 0.1;
void Draw()
{
   ...
   // Render the scene
     ...
     for (int meshIndex = 0;  meshIndex < inputMeshes.size(); ++meshIndex)
     {
		CPoint3d cpoint3d = inputPoints.at(meshIndex); //meshToView
		glPushMatrix(); //Save last matrix
		glTranslatef(cpoint3d.x*explode, cpoint3d.y*explode, cpoint3d.z*explode);


		ListCTriangle3d listTriangls = inputMeshes.at(meshIndex); //meshToView
...
void animate()
{
	if (doAnim)
	{
		...
		explode += explodeAcceleration;
		explodeAcceleration = explodeAcceleration / 1.03;
		glutPostRedisplay();
